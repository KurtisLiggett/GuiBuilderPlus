...
Func _formPropertyInspector($x, $y, $w, $h)
	...
	;create MAIN listview for Ctrls
	$lvPropertiesMain = GUICtrlCreateListView( "", 10, 10, 400, 180, $GUI_SS_DEFAULT_LISTVIEW-$LVS_SINGLESEL, $WS_EX_CLIENTEDGE )
	_GUICtrlListView_SetExtendedListViewStyle( $lvPropertiesMain, $LVS_EX_DOUBLEBUFFER+$LVS_EX_FULLROWSELECT+$LVS_EX_HEADERDRAGDROP )
	$hHeader = _GUICtrlListView_GetHeader( $lvPropertiesMain )
	$hLvPropertiesMain = GUICtrlGetHandle( $lvPropertiesMain
	...
	; Add columns
	Local $editWidth = 80
	Dim $aColLeftEdgePosAcc[2] = [0, $w-$editWidth]
	Dim $aColumnWidths[2] = [$w-$editWidth, $editWidth]

	_GUICtrlListView_AddColumn( $lvPropertiesMain, "Property", $w-$editWidth)
	_GUICtrlListView_AddColumn( $lvPropertiesMain, "Value", $editWidth)
	...
	For $i = 0 To UBound($aLvItems) - 1
		GUICtrlCreateListViewItem( $aLvItems[$i] & "|", $lvPropertiesMain )
	Next

	; Edit control open and close events
	$idEditOpen = GUICtrlCreateDummy()
	GUICtrlSetOnEvent(-1, "_onLvEditOpen")
	$idEditClose = GUICtrlCreateDummy()
	GUICtrlSetOnEvent(-1, "_onLvEditClose")
	...
	_WinAPI_SetWindowSubclass( $hLvPropertiesMain, $pListViewCallback, 9998, 0 )
